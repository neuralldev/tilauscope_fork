...
* **Artisan's menus are simpler.** The View menu and the Sampling, Curves, Phases, Axes, Autosave and Batch settings are no longer offered: TilauScope samples once per second, and Artisan's own graph shows the bean temperature, its projection and its rate of rise. Air temperature is still recorded and still feeds the PID and alarms.
...
* **The crack stays on the curve after the roast.** When a crack probe counted, the Roast Viewer, its snapshot and the roast card show one tick per pop along the foot of the curve — the same band as the roasting window, dense where the crack was.
...
* **A milestone can be corrected on the curve once the roast is over.** Drag it sideways to move it, right-click where one was forgotten to add it, or right-click it to type its time. It never passes its neighbours, the readings never change, and the correction is saved with the roast like any other change.
...
* **Tipping and Scorching can be marked in one click** under the notes of the post-roast form, when you see those burns on the beans. They are saved with the roast, come back when it is reopened, and the AI Summary takes them into account.
