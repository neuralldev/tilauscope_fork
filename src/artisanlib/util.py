...
def fill_gaps(ll, interpolate_max=3):
    arr = numpy.array(ll, dtype=float)
    is_missing = (arr == -1)
    
    # Trouver les indices des valeurs valides
    valid_indices = numpy.where(~is_missing)[0]
    if valid_indices.size == 0:
        return arr.tolist()

    # Remplissage du préfixe
    first_valid_idx = valid_indices[0]
    arr[:first_valid_idx] = arr[first_valid_idx]

    # Identification des "gaps" (trous)
    # On cherche les zones de -1 entre deux valeurs valides
    for start_gap_idx, end_gap_idx in zip(valid_indices[:-1], valid_indices[1:]):
        gap_len = end_gap_idx - start_gap_idx - 1
        if 0 < gap_len <= interpolate_max:
            # Interpolation linéaire vectorisée
            start_val = arr[start_gap_idx]
            end_val = arr[end_gap_idx]
            arr[start_gap_idx+1:end_gap_idx] = numpy.linspace(start_val, end_val, gap_len + 2)[1:-1]
...
# def old_fill_gaps(ll:'Sequence[float|int]|npt.NDArray[numpy.floating[Any]]', interpolate_max:int=3) -> list[float]:
#     res:list[float] = []
#     last_val:float = -1
#     skip:int = -1
...
def getResourcePath() -> str:
    ...
    if platf == 'Darwin':
        if appFrozen():
            return QCoreApplication.applicationDirPath() + '/../Resources/includes/'
...
def toDim(color:str) -> str:
    ...
    if h is not None and s is not None and l is not None and a is not None:
        ## TILAU ##
        gray = QColor.fromHslF(h,s*0.25,(1-l)/1.7+l,a)
    else:
...
def scaleFloat2String(num: float | str) -> str:
    # On suppose que toFloat est déjà optimisé
    n = float(num) 
    if n == 0:
        return '0'
    val_abs = abs(n)
    
    # On utilise des f-strings spécifiques sans rstrip quand c'est possible
    if val_abs >= 1000:
        return f'{n:.0f}'
    
    # Pour les autres, on limite le rstrip à un seul appel si possible
    if val_abs < 10:
        res = f'{n:.3f}'
    elif val_abs >= 100:
        res = f'{n:.1f}'
    else: # Entre 10 et 100
        res = f'{n:.2f}'
...
# def old_scaleFloat2String(num:float|str) -> str:
#     n = toFloat(num)
#     if n == 0:
#         return '0'
#     if abs(n) < 10:
...
def comma2dot(s: str) -> str:
    ...
    # 1. Identifier la position du vrai séparateur décimal (le dernier . ou ,)
    last_dot = s.rfind('.')
    last_comma = s.rfind(',')
    sep_idx = max(last_dot, last_comma)

    # 2. Cas particulier : pas de séparateur ou séparateur à la fin (trailing)
    if sep_idx == -1 or sep_idx == len(s) - 1:
        # On supprime tout simplement les points et virgules
        return s.replace('.', '').replace(',', '')

    # 3. Séparer la partie entière et la partie décimale
    # On nettoie la partie entière (suppression de tous les séparateurs de milliers)
    prefix = s[:sep_idx].replace('.', '').replace(',', '')
    
    # On nettoie la partie décimale (on garde les chiffres, on enlève les 0 inutiles)
    suffix = s[sep_idx + 1:].replace('.', '').replace(',', '').rstrip('0')
...
# def old_float2float(f:float|str, n:int=1) -> float:
#     n = max(n, 0)
#     f = float(f)
#     if n==0:
#         if math.isnan(f):
...
## TILAU ## conversion table built ONCE at module load (full precision, no per-call rebuild)
_LB_PER_KG = 2.20462262185  # international avoirdupois pound per kg
_WEIGHT_CONV_TABLE = (
    #  g                        Kg                      lb                  oz
    (1.,                        0.001,                  _LB_PER_KG/1000,    _LB_PER_KG*16/1000),  # g
    (1000.,                     1.,                     _LB_PER_KG,         _LB_PER_KG*16),       # Kg
    (1000/_LB_PER_KG,           1/_LB_PER_KG,           1.,                 16.),                 # lb
    (1000/(_LB_PER_KG*16),      1/(_LB_PER_KG*16),      1/16,               1.),                  # oz
)

def convertWeight(v: float, i: int, o: int) -> float:
    # explicit bounds: a negative index would silently wrap to a wrong row instead of raising
    if 0 <= i < 4 and 0 <= o < 4:
        return v * _WEIGHT_CONV_TABLE[i][o]
    raise IndexError(f'index error in convertWeight({v},{i},{o})')
 

## TILAU ## i/o: 0:l (liter), 1:gal (gallons US), 2:qt, 3:pt, 4:cup, 5:cm^3/ml
# Définie une seule fois au chargement du script
_VOL_CONV_TABLE = (
    # l            gal             qt              pt              cup             ml
