...
## TILAU ##
        buttonlayout.addWidget(self.btn_visualize)
...
#        self.aw.closeEventSettings()
    ...
    def setalarmtablerow(self, i:int) -> None:
        ...
        flagComboBox.stateChanged.connect(self.onStatusToggled) ## TILAU ##
...
## TILAU ##
    ...
    @pyqtSlot(int)
    def onStatusToggled(self, state: int) -> None:
        """Updates the data model and visual dimming when a Status checkbox is clicked."""
        if self.initializing:
            return
        # We must save the current state of checkboxes to self.aw.qmc.alarmflag 
        # so markNotEnabledAlarmRows reads the new values.

        for i in range(self.alarmtable.rowCount()):
            flag_widget = cast(QCheckBox, self.alarmtable.cellWidget(i, 1))
            if flag_widget:
                self.aw.qmc.alarmflag[i] = 1 if flag_widget.isChecked() else 0
        self.savealarms()
        # Refresh the colors
        self.markNotEnabledAlarmRows()

    # puts a gray background on alarm rows that have already been fired
    # puts a gray background/dimmed text on alarm rows that are fired or disabled
    def markNotEnabledAlarmRows(self) -> None:
        # Define styles
        # We add 'border: 1px solid' to help QLineEdit/QTimeEdit accept the background change
        dim_style = "background-color: rgb(191, 191, 191); color: rgb(80, 80, 80); border: 1px solid rgb(170,170,170);"
        normal_style = "background-color: white; color: black; border: 1px solid rgb(200,200,200);"
        
        dim_color = QColor(191, 191, 191)
        text_dim_color = QColor(80, 80, 80)

        for i in range(self.alarmtable.rowCount()):
            # Logic: dim if alarm already fired (state != -1) OR if status is OFF (flag == 0)
            is_fired = self.aw.qmc.alarmstate[i] != -1
            is_disabled = self.aw.qmc.alarmflag[i] == 0
            should_dim = is_fired or is_disabled
            
            # 1. Dim the Table Items (Background behind the widgets)
            for j in range(self.alarmtable.columnCount()):
                item = self.alarmtable.item(i, j)
                if item is not None:
                    if should_dim:
                        item.setBackground(dim_color)
                        item.setForeground(text_dim_color)
                    else:
                        item.setBackground(Qt.GlobalColor.transparent)
                        item.setForeground(self.palette().color(self.foregroundRole()))

            # 2. Dim the Cell Widgets (QLineEdit, QTimeEdit, QComboBox)
            for j in range(self.alarmtable.columnCount()):
                widget = self.alarmtable.cellWidget(i, j)
                if widget:
                    # Apply style to the widget (or the container for Beep)
                    widget.setStyleSheet(dim_style if should_dim else normal_style)
                    
                    # Beep column (10) uses a QWidget wrapper, we must dim the child checkbox
                    if j == 10:
                        layout = widget.layout()
                        if layout:
                            cb = layout.itemAt(0).widget()
                            if cb:
                                cb.setStyleSheet(dim_style if should_dim else "")

    def createalarmtable(self) -> None:
        try:
            self.initializing = True # Ensure guard is on during rebuild
                ...
                for i, _ in enumerate(self.aw.qmc.alarmtablecolumnwidths):
                    if i not in fixed_columns:
                        try:
                            ...
                            pass
            self.initializing = False # <--- Release the guard here
            self.markNotEnabledAlarmRows()
            self.alarmtable.setSortingEnabled(True)
            self.alarmtable.sortItems(0)
    ...
    def get_current_alarms_data(self):
        """
        Extrait les données du QTableWidget (self.tw) pour reconstruire 
        le dictionnaire attendu par la visualisation.
        """
        data = {
...
## TILAU ##
    # new visualisation logic, autocenters the drawing vertically 2026/02/27
    @pyqtSlot(bool)
    def show_visual_timeline(self, _: bool = False) -> None: 
        from tilauscope.visualalarm import AlarmTimelineDialog
        self.savealarms()
        alarm_source_list  = self.buildAlarmSourceList()
        
        self.visl_timeline = AlarmTimelineDialog(self, alarm_source_list, self.aw)

        # 5. Finally, show the window already centered
        self.visl_timeline.show()