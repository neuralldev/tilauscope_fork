#
# ABOUT
# Acaia scale support for artisan scope
...
#                _log.debug('received: %s',data)
    ...
    ## TILAU ## the scale's own stopwatch, driven by the app's brew timer
    def send_start_timer(self) -> None:
        self.protocol.send_start_timer()

    def send_stop_timer(self) -> None:
        self.protocol.send_stop_timer()

    def send_reset_timer(self) -> None:
        self.protocol.send_reset_timer()
...
class AcaiaBluetooth(Acaia): # pyright: ignore [reportGeneralTypeIssues] # Argument to class must be a base class
    ...
    ## TILAU ## Run the scale's own stopwatch alongside the app's brew timer, so
    ## the number on the scale and the number on screen agree without the
    ## operator starting two timers by hand.
    ## Starting RESETS first: the Acaia stopwatch resumes from where it was
    ## stopped, so a plain start would carry the previous brew's elapsed time
    ## into this one — the scale would read 5:41 on a 3:00 V60.
    @override
    def set_timer(self, on:bool) -> None:
        if on:
            self.acaia.send_reset_timer()
            self.acaia.send_start_timer()
        else:
            self.acaia.send_stop_timer()
...
class AcaiaSerial(Acaia): # pyright: ignore [reportGeneralTypeIssues] # Argument to class must be a base class
    ...
    ## TILAU ## See AcaiaBluetooth.set_timer — same contract, same reset-then-start.
    @override
    def set_timer(self, on:bool) -> None:
        if self.acaia is not None:
            if on:
