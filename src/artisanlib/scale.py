#
# ABOUT
# Scale support for artisan scope
#
...
class STATE_ACTION(IntEnum):
    ...
    ## TILAU ##
    TIMER_ON = 18             # scale timer started
    TIMER_OFF = 19            # scale timer stopped
...
class Scale(QObject):
    ...
    timer_changed_signal = pyqtSignal(int)          ## TILAU ## issued on timer start/stop
...
class ScaleManager(QObject):
    ...
    timer_scale1_signal = pyqtSignal(bool) ## TILAU ## if argument is True, start the scale timer, otherwise stop it
    ...
    timer_scale2_signal = pyqtSignal(bool) ## TILAU ## if argument is True, start the scale timer, otherwise stop it
    ...
    def __init__(self, connected_handler:Callable[[str, str], None], disconnected_handler:Callable[[str, str], None]) -> None:
        ...
        ## TILAU ##
        self.timer_scale1_signal.connect(self.timer_scale1_slot, type=Qt.ConnectionType.QueuedConnection) # type: ignore[call-arg]
        ...
        ## TILAU ##
        self.timer_scale2_signal.connect(self.timer_scale2_slot, type=Qt.ConnectionType.QueuedConnection) # type: ignore[call-arg]
    ...
    def _get_scale(self, model:int, ident:str, name:str) -> Scale|None:
        if model == 0:
            ...
            return AcaiaBluetooth(model, ident, name, lambda : self.connected_handler(ident, name), lambda : self.disconnected_handler(ident, name),
...
#- scale 1
    ...
    def reset_scale1(self) -> None:
        if self.scale1 is not None:
            ...
            try:
                ...
                self.scale1.tare_pressed_signal.disconnect(self.scale1_tare_pressed_slot)
                #-
    ...
    def set_scale1_slot(self, model:int, ident:str, name:str) -> None:
        ...
        if self.scale1 is not None:
            ...
            self.scale1.tare_pressed_signal.connect(self.scale1_tare_pressed_slot)
            #-
    ...
    ## TILAU ##
    @pyqtSlot(bool)
    def timer_scale1_slot(self, on:bool) -> None:
        if self.scale1 is not None:
            self.scale1.set_timer(on)
            if on:
                self.scale1.signal_user(STATE_ACTION.TIMER_ON)
            else:
                self.scale1.signal_user(STATE_ACTION.TIMER_OFF)
...
#- scale 2
    ...
    def reset_scale2(self) -> None:
        if self.scale2 is not None:
            ...
            try:
                ...
                self.scale2.tare_pressed_signal.disconnect(self.scale2_tare_pressed_slot)
                #-
    ...
    def set_scale2_slot(self, model:int, ident:str, name:str) -> None:
        ...
        if self.scale2 is not None:
            ...
            self.scale2.tare_pressed_signal.connect(self.scale2_tare_pressed_slot)
            #-
    ...
    ## TILAU ##
    @pyqtSlot(bool)
    def timer_scale2_slot(self, on:bool) -> None:
        if self.scale2 is not None:
            self.scale2.set_timer(on)
            if on:
                self.scale2.signal_user(STATE_ACTION.TIMER_ON)
            else:
                self.scale2.signal_user(STATE_ACTION.TIMER_OFF)
