#
# ABOUT
# A simple device simulator for artisan scope
#
...
class Simulator:
    __slots__ = [ 'profile', 'temp1', 'temp2', 'timex', 'extratemp1', 'extratemp2', 'extratimex',
        ...
        'tilau_no_curve'] ## TILAU ## True when the profile has no BT curve to replay
    ...
    def removeEmptyPrefix(self) -> None:
        ## TILAU ## A profile with no valid BT (empty temp2 or all -1) has no curve
        ## to replay. Always build the *_array attributes (empty in that case) and
        ## raise tilau_no_curve, instead of bailing out mid-way on the ValueError
        ## from .index(True) — which left the arrays unset so every read() failed
        ## silently and returned -1, zeroing every LCD. ## TILAU ##
        self.tilau_no_curve = False
        try:
            # select the first BT index not an error value to start with
            start = next((i for i, v in enumerate(self.temp2) if v != -1), None)
            if start is None:
                self.tilau_no_curve = True
                self.temp1_array = numpy.array([])
                self.temp2_array = numpy.array([])
                self.timex_array = numpy.array([])
                self.extratemp1_array = []
                self.extratemp2_array = []
                self.extratimex_array = []
                _log.warning('Simulator: profile has no valid BT curve — nothing to replay')
                return
