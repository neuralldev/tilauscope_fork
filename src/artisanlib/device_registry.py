...
# (check also the tags in canvas.py, comm.py and devices.py!!)
# - add to DEVICES
# NOTES:
#  - the index of the elements in this list is one less than the device id indicated in the comment below
...
DEVICES: Final[list[str]] = [
    ...
    'Omega HH802U',          #16
    'Omega HH309',           #17
    'NONE',                  #18 (DEFAULT; manual BT/ET input)
    '-ARDUINOTC4',           #19
    'TE VA18B',              #20
    '+CENTER 309 34',        #21
    '+PID SV/DUTY %',        #22
...
NON_TEMP_DEVICES : Final[dict[int,tuple[bool,bool]]] = {
            # channel 1, channel 2
...
NON_SERIAL_DEVICES: Final[set[int]] = PHIDGET_DEVICES.union({
    18, # NONE (manual)
    27, # Program
...
def is_non_temp_device_chan(device_id: int, chan:int) -> bool:
    return device_id in NON_TEMP_DEVICES and NON_TEMP_DEVICES[device_id][chan]
...
def get_device_name(device_id: int) -> str:
    return DEVICES[max(0, device_id - 1)]
...
## TILAU ##
# TilauScope devices extend the upstream registry at import time so their ids,
# DEVICE_ID_MAX and the temp/serial lookups below are computed over them too.
# The literal "id" values are placeholders: the loop below rewrites each one to
# its final position in DEVICES, which is the only id Artisan persists.
TILAU_DEVICES: dict[str, dict] = {
    'difluid': {'id': 190, 'label': 'Difluid AirWave', 'nonserial': True, 'nonTemp': (False, False)}, # inlet and catalyst temp
...
_tilau_first_id: int = len(DEVICES) + 1
for _i, _tilau_device in enumerate(TILAU_DEVICES.values()):
    _tilau_device['id'] = _i + _tilau_first_id
DEVICES.extend(d['label'] for d in TILAU_DEVICES.values())
NON_TEMP_DEVICES.update({d['id']: d['nonTemp'] for d in TILAU_DEVICES.values()})
# only entries actually marked nonserial (BLE/MQTT); a genuinely serial TRP entry
# keeps the standard comport/baud config fields (see ports.py)
...
DEVICE_ID_MIN:Final[int] = 0     # lowest valid device id
DEVICE_ID_MAX:int = len(DEVICES) # highest valid device id
DEVICE_ID_NONE:int = get_device_id('NONE') # 18
DEVICE_ID_VIRTUAL:int = get_device_id('+Virtual') # 25
