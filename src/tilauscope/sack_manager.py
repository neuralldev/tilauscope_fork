...
class _SackBatchPrintWorker(QObject):
    ...
    unconfirmed = pyqtSignal(int)     # labels counted as used, the unconfirmed one included
    ...
    def run(self) -> None:
        from tilauscope.niimprint import NiimbotPrintOutcome  # noqa: PLC0415
        printed = 0
        try:
            for img in self._images:
                if self._stop:
                    break
                outcome = self._printer.print_image(img, 3, self._type)
                if outcome is NiimbotPrintOutcome.UNCONFIRMED:
                    # It may be out: count it, so its number never goes to another label.
                    self.unconfirmed.emit(printed + 1)
                    return
                if outcome is not NiimbotPrintOutcome.PRINTED:
                    self.error.emit(self._printer.print_failure, printed)
                    return
                printed += 1
...
class SackLabelsDialog(QDialog):
    ...
    def _print_ids(self, ids: list[str]) -> None:
        ...
        self._worker.unconfirmed.connect(self._on_print_unconfirmed)
        ...
        try:
            ...
            if worker is not None:
                slots = ((worker.progress, self._on_print_progress),
                         ...
                         (worker.unconfirmed, self._on_print_unconfirmed))
    ...
    def _on_print_unconfirmed(self, counted: int) -> None:
        from PyQt6.QtWidgets import QMessageBox
        from tilauscope.niimprint import unconfirmed_pill_text  # noqa: PLC0415
        # The label may be out: it counts as used, so its number never goes to another bag.
        self._finish_run(counted)
        ids = getattr(self, '_run_ids', [])
        sack_id = ids[counted - 1] if 0 < counted <= len(ids) else ""
        pill = self._print_pill
        self._print_pill = None
        if pill is not None:
        ...
        show_styled_message(self,
            QApplication.translate("tilauscope_sacks", "Sack ID labels"),
            QApplication.translate("tilauscope_sacks",
                "Check label {0} — the printer did not confirm it. If it came out blank, "
                "print it again from the Reprint tab: its number stays reserved, so no "
                "other bag can get it.").format(sack_id),
            QMessageBox.Icon.Warning)
