...
from tilauscope.tilau_privacy import host_is_public, prepare_ai_messages, sanitize_url
...
_MAX_REDIRECTS   = 5    # hops followed before the address is given up on
_MAX_PAGE_BYTES  = 4_000_000  # a product page that big is not one
...
def _fetch_public(url: str) -> str:
    """Fetch *url* as text, refusing any hop that leaves the public internet.

    requests follows redirects on its own, so the check on the pasted address
    only ever covered the first hop: a supplier page redirecting to a local one
    was fetched from the operator's own network and forwarded to the model.
    Every hop is re-checked here, address and DNS answer alike, and the body is
    capped so a hostile page cannot fill memory.
    """
    for _hop in range(_MAX_REDIRECTS + 1):
        ...
        url = verdict.url
        if not host_is_public(urlparse(url).hostname or ""):
            raise RefusedURL("private-host")
        ...
        try:
            ...
            response.close()
        return b"".join(chunks)[:_MAX_PAGE_BYTES].decode(encoding, errors="replace")
    raise RefusedURL("unusable")
...
def scrape_url(url: str) -> tuple[str, str]:
    ...
    # 1. JSON-LD — read BEFORE the noise removal below, which strips every
    # script element, this source included.
