...
class PIDAutotune(QDialog):
    ...
    def is_stable(self, window_size: int = 5, max_spread: float = 0.4) -> bool:
        ...
        window_size : nombre de points temp2 à considérer (chaque point ≈ 1 s dans Artisan).
        max_spread  : écart max toléré (°C) — 0.4°C est raisonnable pour du bean-temp.
        La méthode est désormais appelée dans update_logic pour inhiber les ajustements
        qui requièrent de la stabilité.
        """
    ...
    def setup_ui(self):
        ...
        help_btn.setStyleSheet("border-radius: 12px; background-color: #3498db; color: white; font-weight: bold;")
    ...
    def add_row(self, group, label_text):
        ...
        field_lbl.setStyleSheet(f"color: {THEME['SUBTEXT']}; border: none;") # Set color here
    
        # The value label (the text on the right)
        lbl = QLabel("-")
        lbl.setStyleSheet(f"color: {THEME['TEXT']}; font-family: 'JetBrains Mono'; border: none;")
    ...
    def get_active_pid_slots(self, bt:float):
        ...
        # Added safety: Check if attributes exist before returning them
        try: # 2026/04/01 -  fixed bug
    ...
    def _adjust_kp(self, kp_val: float, error: float, attr_p: str, attr_i: str, attr_d: str) -> str:
        ...
        exponentielle. Le pas est proportionnel à l'erreur résiduelle, avec borne.
        Retourne le libellé action.
        """
    ...
    def _preheat_adjust(
        ...
        """
        Ajustements preheating avec bornes strictes.
        - Kd : montée douce (+5 par cycle max), plafond à 300
        - Kp : descente douce (-0.3 par cycle max), plancher à 4
        """
    ...
    def _near_target_threshold(self, bt: float) -> float:
        """
        Seuil d'erreur (°C) pour considérer qu'on est "près de la consigne".
        La tolérance est réduite à mesure que BT monte car la masse thermique
        du café et la réactivité du torréfacteur changent avec la température.
    
        Plages (mode post-chargement) :
        bt ≤  95°C  → ±4.0°C  (phase Turning Point, dynamique rapide)
        bt ≤ 150°C  → ±3.0°C  (phase Maillard, dynamique plus lente)
        bt >  150°C → ±2.0°C  (finition, précision maximale demandée)
        """
        if bt <= 95.0:
            return 4.0
        if bt <= 150.0:
