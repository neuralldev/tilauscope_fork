...
def live_beancave(bc: object | None) -> bool:
    """True when an existing BeanCave can simply be shown again.

    Closing one tears down its printer and probes, its selection wiring and its
    tooltip, and nothing rebuilds them on show: a closed BeanCave is replaced
    by a new one, never reused.
    """
    return bc is not None and not sip.isdeleted(bc) and not getattr(bc, 'is_shutting_down', False)
...
class LifecycleMixin:
    ...
    def __init__(self, aw:'ApplicationWindow', uuid:str="") -> None:
        ...
        self._niimbot_silent = False  # True while the printer ignores its heartbeat
        ...
        # Guards the datatable <-> plan-combo mirror against re-entering itself.
        self._syncing_bean_selection: bool = False
        ...
        # Assigned, not merely annotated: the Roast Viewer enables its actions
        # as soon as the list is painted, before the first async load lands.
        self.lastprofiledata:ProfileData|None = None
        # Ambient lookup (weather / probe) — one at a time, joined at teardown.
        self._ambient_thread = None
        self._ambient_worker = None
        self._alog_cache: dict[str, tuple[float, ProfileData]] = {}  # LRU cap = ALOG_CACHE_MAX
        # The cache is filled from the load thread as well as the GUI thread.
        self._alog_cache_lock = threading.Lock()
    ...
    def bean_uuid_from_profile(self, profile) -> str:
        ...
        try:
            m = self.uuid_pattern.search(str((profile or {}).get('beans', '') or ''))
            return m.group(1) if m else ''
        except Exception as e:  # noqa: BLE001  pylint: disable=broad-except
            _logd.debug(f"bean uuid resolution skipped: {e}")
            return ''

    def bean_from_profile(self, profile):
        """The live green-bean record behind a roast profile, or None.

        The same three steps — read ``beans``, pull the uuid out of it, look it
        up in the uuid map — were written out at four separate sites.
        """
        uuid_str = self.bean_uuid_from_profile(profile)
        if not uuid_str:
            return None
        return getattr(self, 'uuidmap', {}).get(uuid_str)
    ...
    def _cancel_threads(self) -> None:
        ...
        # --- Folder scan ---
        # Same parenting, same fatal end: a scan of a slow or large directory
        # still running when the dialog is destroyed aborts the process.
        self._stop_list_scan()

        # --- Ambient lookup (weather / probe) ---
        if getattr(self, '_ambient_thread', None) is not None:
            # Which slots were wired depends on which lookup ran, so drop every
            # receiver rather than naming them: a reading landing during the
            # wait would reach a half-destroyed dialog.
            # Reading an attribute off a worker Qt already deleted raises
            # RuntimeError, which getattr does not catch: same rule as the
            # timers above, a raise here abandons every thread below.
            try:
                for signal_name in ('finished', 'error'):
                    sig = getattr(self._ambient_worker, signal_name, None)
                    if sig is None:
                        continue
                    try:
                        sig.disconnect()
                        ...
                        pass
                stop_worker_thread(self._ambient_thread, self._ambient_worker,
                                   name="ambient lookup")
            except RuntimeError:
                pass  # already collected by Qt
            self._ambient_thread = None
        ...
        # --- Niimbot silence check (timer only; the keep-alive runs on the BLE loop) ---
        # Stoppe le timer + déconnecte status_updated AVANT np.stop().
    ...
    def setup_ui(self) -> None:
        ...
        for _cb in (
            ...
            # The dialog's own filter, like every other widget here. A filter
            # per combo, stored under id(combo), duplicated the mechanism and
            # kept keys that id() reuses once a widget is freed.
            self.hover_filter.install(_cb)
    ...
    def _close_step_table_signals(self) -> None:
        ...
        # left both slots live for the whole teardown.
        try:
            self.datatable.itemSelectionChanged.disconnect()
        except (TypeError, RuntimeError):
