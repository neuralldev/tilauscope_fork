...
from tilauscope.cave.widgets import (
    QRCodeDialog)
from tilauscope.cave.workers import NiimbotWorker
...
class PrintingMixin:
    ...
    def _start_niimbot_poll(self) -> None:
        """Starts the 5 s check that notices a printer gone silent."""
    ...
    def _on_niimbot_poll_tick(self) -> None:
        """Every 5 s: say so when the printer has stopped answering.

        The keep-alive itself runs on the printer's BLE loop (NiimbotBLE.heartbeat)
        and its answers reach _on_niimbot_status; this only compares timestamps.
        No BLE call from here: a throwaway QThread calling the printer hung
        BeanCave once in ten opens.
        """
        if self.np is None or not self._niimbot_ble_up:
            return
        if self._niimbot_silent or not self.np.is_silent(time.monotonic()):
            return
        _log.warning("niimbot printer stopped answering")
        self._niimbot_silent = True
        self._niimbot_connected = False
        self.niimbot_overlay.update_status(
    ...
    def _on_niimbot_status(self, hb:NiimbotHeartbeat, rfid:NiimbotRFIDinfo) -> None:
        ...
        if not hb.valid:
            return
        self._niimbot_silent = False   # it answered
    ...
    def generate_and_print_label(self) -> None:
        ...
        filepath = self.selected_roast_path()
        if filepath is None:
            return
    ...
    def _on_print_progress(self, done: int, total: int) -> None:
        ...
        The label is sent once, then the printer burns the copies: the ring fills
        once over the whole run, and is full only when the last one is reported out.
        """
        pill = getattr(self, "_print_pill", None)
        if pill is None or total <= 0:
            return
        from tilauscope.niimprint import copies_progress  # noqa: PLC0415
        copies = max(1, int(getattr(self, "_roast_print_copies", 1)))
        pill.progress.setRange(0, copies)   # a single label's pill starts as a sweep
        self._roast_print_sent = done / float(total)
        pill.progress.set_value(copies_progress(
            self._roast_print_sent, int(getattr(self, "_roast_print_out", 0)), copies))

    @pyqtSlot(int, int)
    def _on_roast_copy_progress(self, out: int, n: int) -> None:
        """Worker signal each time the printer reports one more copy out."""
        self._roast_print_out = out
        ...
        from tilauscope.niimprint import copies_progress  # noqa: PLC0415
        if n > 1:
            pill.set_step(out, n)   # a single label has nothing to count
        pill.progress.set_value(copies_progress(
    ...
    def _on_print_error(self, message):
        ...
        show_styled_message(self,
                            ...
                            QApplication.translate(
                                ...
                                "is loaded, then try again.").format(message),
                            QMessageBox.Icon.Warning)

    def _on_print_unconfirmed(self) -> None:
        from tilauscope.niimprint import unconfirmed_message, unconfirmed_pill_text  # noqa: PLC0415
        self._roast_print_copies = 1
        self._roast_print_out = 0
        QTimer.singleShot(500, self._on_niimbot_poll_tick)
        pill = getattr(self, "_print_pill", None)
        self._print_pill = None
        if pill is not None:
            pill.fail("🖨  " + unconfirmed_pill_text())
        show_styled_message(self,
                            QApplication.translate("tilauscope_beancave","Niimbot B21S Print"),
                            unconfirmed_message(), QMessageBox.Icon.Warning)

    def print_niimbot_image_async(self, img, on_finished, on_error, on_unconfirmed=None) -> None:
        ...
        thread. ``on_unconfirmed()`` runs when the printer never confirmed the
        label; without it, ``on_error`` receives the short "not confirmed" text.
        BLE access is serialised by NiimbotBLE's internal lock, so this
        ...
        if on_unconfirmed is None:
            from tilauscope.niimprint import unconfirmed_pill_text  # noqa: PLC0415
            on_unconfirmed = lambda: on_error(unconfirmed_pill_text())  # noqa: E731
        self._sat_niimbot_worker.print_unconfirmed.connect(on_unconfirmed)
    ...
    def on_print_label_clicked(self):
        ...
        if bean:
            ...
            file_path = self._open_file_dialog_save(
                QApplication.translate("tilauscope_beancave", "Save Label PDF"), default_name,
