...
class ViewerMultiMixin:
    ...
    def _multi_milestones(self, data: dict) -> dict:
        ...
        # 2 min converted through this log's own sampling interval: 120 was being
        # read as 120 samples, which is six minutes on a 3 s log and let a much
        # later BT dip pass for the turning point.
        tp_span = max(1, int(round(120.0 / estimate_ror_dt(timex))))
        tp_hi = dryend if dryend > charge else min(len(temp2), charge + tp_span)
    ...
    def _plot_multi_curves(self) -> None:
        ...
        if align:
            ...
            # NaN survives when the RoR limit is off, and one NaN makes the whole
            # max NaN — set_ylim then raises, from a timer slot, outside any guard.
            all_ror = [v for s in self._multi_series for v in (s['ror'] or [])
