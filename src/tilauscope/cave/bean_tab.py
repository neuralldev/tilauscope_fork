...
from tilauscope.cave.common import (
    ...
    _atomic_write_text, NUMERIC_COLUMNS, set_combo_value)
...
def BLEND_COMPONENT_PLACEHOLDER() -> str:
    """The "nothing selected" entry of the blend component combos.

    A function, not a constant: it is translated, and the catalogue is not
    loaded yet when this module is imported.
    """
    return QApplication.translate("tilauscope_beancave", "N/A - Select a bean")


def _blend_component_name(combo) -> str:
    """The component chosen in a blend combo, empty when it is the placeholder."""
    text = combo.currentText()
    return '' if text == BLEND_COMPONENT_PLACEHOLDER() else text
...
class BeanTabMixin:
    ...
    def _update_variety(self, var: str) -> None:
        if hasattr(self, 'varieties_combo'):
            ...
            try:
                ...
                # Logged, not swallowed: a missing species key leaves a combo
                # holding nothing but "Other", with no trace of why.
                _logd.warning(f"no varieties listed for species '{var}': {e}")
    ...
    def _update_methods(self, cat: str) -> None:
        if hasattr(self, 'process_combo'):
            ...
            try:
                ...
                _logd.warning(f"no processing methods listed for category '{cat}': {e}")
    ...
    def _update_blend_component_list(self) -> None:
        """Populates the component comboboxes with the blend component varieties."""
        ...
        # The placeholder leads the list in every case. Adding it only when the
        # list was empty meant a blend with no component set showed, and saved,
        # whichever variety happened to sort first — indistinguishable from a
        # deliberate choice. The rest of this class already assumes it is there.
        single_origin_names.insert(0, BLEND_COMPONENT_PLACEHOLDER())
    ...
    def update_ui_visibility(self) -> None:
        ...
        # The whole tab content, not just the "Origin & Identity" box: hiding
        # that one left Botany, Blend, Physical measures, Computed and the notice
        # bar on screen, with the empty-state message underneath them.
        if hasattr(self, '_main_splitter'):
            self._main_splitter.setVisible(show_form)
    ...
    def populate_table(self) -> None:
        ...
        # Which bean was selected, so the rebuild can put the operator back on it.
        # Restoring this was left to each caller, and most did not: deleting a
        # bean, refreshing the home view or saving a zone editor dropped the
        # selection to the first row, and the next Roast, Delete or Label acted
        # on that one instead.
        previous_uuid = ''
        ...
        if cur is not None:
            ...
            if anchor is not None:
                previous_uuid = anchor.data(Qt.ItemDataRole.UserRole) or ''
        ...
        for row, bean in enumerate(beans):
            for col, value_fn in enumerate(GREEN_BEAN_COLUMNS):
                ...
                # Right-align the numeric columns. This used to write the parsed
                # float under EditRole, which QTableWidgetItem also serves as the
                # displayed text: the species, flavour-notes and blend columns
                # were listed as "0.0", and two real numeric columns were missing
    ...
    def load_selected_bean_into_form(self) -> None:
        ...
        if row < len(self.cave.green_beans):
            ...
            set_combo_value(self.country_combo, bean.country)
            self.supplier_input.setText(bean.supplier)

            set_combo_value(self.category_process_combo, bean.category)
            self._update_methods(bean.category)
            set_combo_value(self.process_combo, bean.process)
            ...
            set_combo_value(self.species_combo, bean.species)
            self._update_variety(bean.species)
            set_combo_value(self.varieties_combo, bean.varieties)
            ...
            bean2_text = bean.bean2_name if bean.bean2_name else BLEND_COMPONENT_PLACEHOLDER()
            ...
            bean3_text = bean.bean3_name if bean.bean3_name else BLEND_COMPONENT_PLACEHOLDER()
    ...
    def selected_green_bean(self):
        """The green bean the table selection points at, or None.

        Every caller used to index ``cave.green_beans`` straight from a row
        number: with no library loaded, or a row left selected after a failed
        reload, that raises inside a clicked slot and closes the application.
        """
        if self.cave is None or not hasattr(self.cave, 'green_beans'):
            return None
        rows = self.datatable.selectionModel().selectedRows()  # type: ignore
        row = rows[0].row() if rows else self.datatable.currentRow()
        if row is None or row < 0 or row >= len(self.cave.green_beans):
            return None
        return self.cave.green_beans[row]
    ...
    def update_selected_bean(self) -> None:
        ...
        if selected_row_index < len(self.cave.green_beans):
            ...
            new_bean_data = GreenBean(
                ...
                bean2_name=_blend_component_name(self.bean2_combo) if is_blend_selected else '',
                bean2_ratio=self.bean2_ratio_input.value() if is_blend_selected else 0.0,
                bean3_name=_blend_component_name(self.bean3_combo) if is_blend_selected else '',
    ...
    def save_green_beans(self) -> bool:
        """True only when the library actually reached the disk.
    ...
    def clear_form(self) -> None:
        ...
        # Index -1, not 0: none of these lists opens on a neutral entry, so
        # clearing the form used to answer "Angola", "Traditional Dry" and
        # "Arabica" as though the operator had chosen them.
        self.country_combo.setCurrentIndex(-1)
        self.supplier_input.clear()
        self.category_process_combo.setCurrentIndex(-1)
        ...
        self.varieties_combo.setCurrentIndex(-1)
        self.weight_left_input.setValue(0.0)
        self.weight_input.setValue(0.0)  # roasted total; a cleared form kept the previous bean's
    ...
    def on_click_ai_parse(self):
        # One extraction at a time. A second press replaced the thread, worker
        # and progress handles of the first: the running thread lost its last
        # Python reference, teardown could no longer join it, and its progress
        # dialog stayed on screen for good.
        existing = getattr(self, 'ai_thread', None)
        try:
