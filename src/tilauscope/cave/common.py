...
#: Parsed profiles kept in memory. The comparison view loads up to five
#: roasts and the single view holds one more, so a cap of five evicted a
#: roast on every switch between the two.
ALOG_CACHE_MAX: Final[int] = 8

#: Columns of the green-bean table holding a number, right-aligned on display.
#: Indices follow GREEN_BEAN_COLUMNS: crop, density, humidity, water activity,
#: altitude, stock left, SCA score, roast count, roasted weight.
NUMERIC_COLUMNS: Final[tuple[int, ...]] = (6, 7, 8, 9, 10, 13, 15, 16, 17)


def set_combo_value(combo, value: object) -> None:
    """Select ``value`` in a combo, or clear the selection when it has no entry.

    ``setCurrentText`` on a non-editable combo silently does nothing when the
    text is absent from the list, so a bean with no country, or one whose
    process the list does not carry, kept showing the previously selected bean's
    answer — and the form is what the record is written back from.
    """
    text = '' if value is None else str(value)
    idx = combo.findText(text) if text else -1
    combo.setCurrentIndex(idx)


def ror_span_samples(qmc, data: dict, deltaname: str) -> int:
    """RoR differentiation span, in samples of THIS log.

    The live counter (``qmc.deltaBTsamples``) is sized for the recording
    session, not for a stored log: differentiating a 1 Hz log over it while the
    app is set to 3 s sampling collapses a 15 s span to 5 s and invents noise
    the roast never had. Both the GUI path and the loader thread size the span
    here so they cannot drift apart.
    """
    log_dt = estimate_ror_dt(data.get('timex', []))
    span = qmc.deltaETspan if deltaname == 'temp1' else qmc.deltaBTspan
    return max(1, int(round(span / log_dt)))


def recompute_profile_deltas(qmc, data: dict, deltaname: str, ds: int):
    """The RoR series for one channel of a stored profile, or None.

    A profile carries its unit under ``mode``; ``temp_unit`` only exists in the
    report dict Artisan builds for exports. Reading the missing key defaulted
    every log to Celsius and pushed an °F roast through a C→F conversion before
    differentiating it. An absent ``mode`` now means "no conversion" rather than
    a guess.
    """
    ...
    unit = str(data.get('mode', '') or '')
    temp = [convertTemp(t, unit, qmc.mode) for t in data.get(deltaname, [])]

    cf = qmc.curvefilter  # smoothed twice as heavy for PID/RoR as for the drawn curve
    t1 = smooth_list(data.get('timex', []),
                     (fill_gaps(temp) if qmc.interpolateDropsflag else temp),
                     window_len=cf, decay_smoothing=not qmc.optimalSmoothing)
    if len(t1) > 10 and len(tx) > 10:
        # RoR computation starts 10 readings after CHARGE to skip the initial peak
        ror_start = min(rd + 10, len(tx) - 1)
        _, deltas = qmc.recomputeDeltas(tx, ror_start, drop, None, t1,
                                        optimalSmoothing=qmc.optimalSmoothing,
                                        deltaBTsamples=ds)
        return deltas
    return None
