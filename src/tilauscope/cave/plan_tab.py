...
class PlanTabMixin:
    ...
    def _on_plan_bean_changed(self, index: int) -> None:
        ...
        # Mirror the selection in the main-tab datatable, signals live. Blocking
        # them moved the row without reloading the form, and the form is what
        # update_selected_bean() writes back: the next stock decrement then
        # overwrote the newly selected bean with the previous one's fields.
        # The guard breaks the loop the live signals open, since reloading the
        # form calls _update_roast_plan_ui_state() straight back into here.
        if self.datatable.rowCount() > index and not self._syncing_bean_selection:
            self._syncing_bean_selection = True
            try:
                self.datatable.selectRow(index)
            finally:
                self._syncing_bean_selection = False
    ...
    def setup_roast_plan_tab_ui(self) -> None:
        ...
        for i, label in enumerate(milestones):
            ...
            start_input.setSuffix(QApplication.translate("tilauscope_beancave", " min"))
            ...
            end_input.setSuffix(QApplication.translate("tilauscope_beancave", " max"))
