...
class RoastDataBridge(QObject):
    ...
    sample_ready           = pyqtSignal()               # échantillon complet publié
    ...
    def tick(self, _data: int | str = 10) -> None:
        ...
        # Ordre volontaire : les trois mesures sont publiées AVANT le signal qui
        # déclenche le calcul. Émettre la BT en premier et laisser son slot
        # rafraîchir la page faisait lire l'ET et le RoR de l'échantillon
        # PRÉCÉDENT — connexions directes, tout est synchrone — et une BT
        # inchangée ne rafraîchissait rien du tout.
        changed = self._emit_bt()
        changed = self._emit_et() or changed
        changed = self._emit_ror() or changed
        self._check_ambient()
    ...
    def _emit_bt(self) -> bool:
        """Publie la BT ; renvoie True si la valeur a changé ce tick."""
        try:
    ...
    def _emit_et(self) -> bool:
        """Publie l'ET ; renvoie True si la valeur a changé ce tick."""
        try:
    ...
    def _emit_ror(self) -> bool:
        """Publie le RoR ; renvoie True si la valeur a changé ce tick."""
        try:
