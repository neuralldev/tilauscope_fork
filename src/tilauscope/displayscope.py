...
class TilauScope(BuildMixin, ChromeMixin, LiveMixin, SlidersMixin, MilestonesMixin, LifecycleMixin, EmergencyMixin, QWidget):
    ...
    def _apply_operator_level(self, level: str, from_init: bool = False,
        ...
        if not is_guided and (self._assistant_anchored or self._assistant_open):
            # Expert has no assistant at all — not one opened by hand. A docked
            # panel gives the control panel back and a detached one comes down
            # with it: left floating, it stayed on screen at a level that says
            # there is none, still flagged active, and the 1 Hz loop that feeds
            # it only runs while it is on screen or docked.
            self._assistant_anchored = False
            self._assistant_open = False
            if persist:
                QSettings().setValue("tilauscope/assistant_anchored", False)
            # A session with no panel to draw on is a plan nothing advances.
            try:
                self.roast_assistant.stop_session()
            except Exception:  # pylint: disable=broad-except
                _log.exception("stopping the assistant for Expert failed")
    ...
    def toggle_beancave(self):
        ...
        if hasattr(self.aw, 'beancaveWindow') and self.aw.beancaveWindow is not None and self.aw.beancaveWindow.isVisible():
            ...
            # keeping its indexer thread and BLE handles alive until quit. A closed
            # one is replaced: its teardown (printer, selection, tooltip) is not undone on show.
            bc = getattr(self.aw, 'beancaveWindow', None)
