...
class LifecycleMixin:
    ...
    def _borrow_expert_level(self) -> None:
        ...
        if self._level_before_replay is None:
            ...
            # Open/closed is remembered with it: docking the assistant back
            # opens it, so restoring the anchor alone ended a replay by
            # raising a window the operator had put away.
    ...
    def toggle_roast_assistant(self):
        ...
        # Expert has no assistant. The header buttons that would open or dock
        # one are hidden, so the rule is enforced here instead of at each of
        # them: this is the single point every way in goes through, the
        ...
        # instead and the first press changed nothing on screen. A DETACHED
        # assistant is its own window and never wanted the column, so the
        # review stays where it is and the close below applies.
        _take_back = (getattr(self, '_review_shown', False)
                      and getattr(self, '_assistant_anchored', False))
        # Closing a DETACHED assistant means the same thing whether its own ✕
        # or this button asked, so both go through the one method that knows
        # what closing means at each level: Guided sends it back to its dock
        # with the session intact — float ↔ dock is a placement choice, not an
        # open/close one — and Expert ends the session rather than leaving it
        # running with nowhere to draw. Either way nothing is left active and
        # unfed: the 1 Hz loop that carries the guidance, the crash watch and
        # the AutoPilot only runs while the assistant is on screen or docked.
        if (getattr(self, '_assistant_open', False)
                and not getattr(self, '_assistant_anchored', False)):
            self.roast_assistant.hide_from_button()
            return
        if _take_back:
