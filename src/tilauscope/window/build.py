...
class BuildMixin:
    ...
    @staticmethod
    def _resolve_operator_level() -> str:
        """The stored operator level, normalised to one of the two that exist.

        Guided is the default: new installs, and the legacy "standard" of an
        older install, both land there. The normalised value is written back
        so nothing downstream has to know about the levels we no longer have.
        """
        level = QSettings().value("tilauscope/operator_level")
    ...
    def _declare_state(self) -> None:
        ...
        # generation of the commands the sliders are allowed to send. The heat
        # cut bumps it so a click still sitting in its debounce window cannot
        # land afterwards; each slider remembers the generation it scheduled in.
        self._slider_command_epoch: int = 0
        self._slider_commit_epochs: dict[int, int] = {}
        ...
        # Resolved here rather than after the build: _build_panels() defaults
        # the assistant's placement from it, and it used to be read only once
        # every panel already existed.
        self._operator_level: str = self._resolve_operator_level()
        self._level_before_replay: str | None = None
        self._anchor_before_replay: bool | None = None
        self._open_before_replay: bool | None = None
    ...
    def init_ui(self):
        ...
        # ── Operator level init ──────────────────────────────────
        # The level itself was resolved in _declare_state(), before the panels
        # that read it. Only the widgets are brought in line here.
        self._apply_operator_level(self._operator_level, from_init=True)
    ...
    def _build_panels(self) -> None:
        ...
        # No placement stored yet: the level says what the window opens on —
        # Guided is the assistant in place of the control panel, Expert is the
        # control panel. Once the operator has moved it, their choice wins.
        self._assistant_anchored: bool = QSettings().value(
            "tilauscope/assistant_anchored",
            self._operator_level == "guided", type=bool)
