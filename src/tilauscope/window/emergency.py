...
class EmergencyMixin:
    ...
    def _emergency_slider_targets(self) -> list[tuple[int, int, str]]:
        ...
        # One slider, one setpoint. The burner is added first and keeps its
        # index: airflow is a position (0) and extraction an action, so on a
        # machine whose burner sits on either of them the later target would
        # otherwise drive the burner back to maximum right after cutting it.
        claimed: set[int] = set()

        def _add(idx: int | None, value: int, name: str) -> None:
            if idx is None or idx in claimed:
                if idx is not None:
                    _log.warning('emergency: slider %s already commanded — '
                                 '%s skipped', idx, name)
                return
            try:
    ...
    def _command_safe_state(self) -> tuple[list[str], list[str]]:
        ...
        commanded: list[str] = []
        manual: list[str] = []
        applied: list[tuple[int, int]] = []   # what actually reached the machine
        signal = getattr(self.aw, 'tilaupidSliderCommandSignal', None)
        fire = getattr(self.aw, 'simulator', None) is None
        targets = self._emergency_slider_targets()
        # Drop what the operator clicked in the last 300 ms before pressing the
        # cut: those commands are still waiting in their debounce window, and
        # committing them afterwards would put the heat back on.
        try:
            self.discard_pending_slider_commits()
        except Exception:  # pylint: disable=broad-except
            _log.exception('emergency: discarding pending slider commands failed')
        for idx, value, name in targets:
            try:
                ...
                applied.append((idx, value))
            except Exception:  # pylint: disable=broad-except
                _log.exception('emergency: safe command failed on slider %s', idx)
                manual.append(name)
        # Show the safe setpoint on the handles too: a handle still held down
        # fences off the mirror from Artisan, and releasing it would schedule
        # the value under the finger all over again. Only the levers that were
        # really commanded: a lever reported as manual received nothing, and
        # showing it safe would contradict the instruction given for it.
        for idx, value in applied:
            try:
                self.pin_slider_value(idx, value)
            except Exception:  # pylint: disable=broad-except
                _log.exception('emergency: pinning slider %s failed', idx)
