...
class SlidersMixin:
    ...
    def handle_ui_input_released(self, n, immediate: bool = False):
        if self.init or self._syncing_from_artisan:
            return
        # Stamp the generation this gesture belongs to: the commit checks it
        # back, so a click scheduled before a heat cut cannot reach the machine
        # after it, while a gesture made afterwards still goes through.
        self._slider_commit_epochs[n] = self._slider_command_epoch
    ...
    def discard_pending_slider_commits(self) -> None:
        """Drop every debounced slider command without sending it.

        The opposite of :meth:`flush_pending_slider_commits`, and the one the
        heat cut needs: flushing would push the operator's last click onto a
        machine that has just been commanded safe.
        """
        self._slider_command_epoch += 1
        for timer in self._slider_commit_timers.values():
            timer.stop()
        # The stamps are deliberately kept: they are what a commit compares
        # itself against, and clearing them would let a timer that fires anyway
        # read the new generation as its own.

    def pin_slider_value(self, n, value) -> None:
        """Force slider `n` to show `value` without sending anything outward.

        Used by the heat cut: the mirror from Artisan stands back while a handle
        is held down, so the safe setpoint has to be written onto the widget
        itself or releasing the handle would schedule the old value again.
        """
        slider = self._slider(n)
        if slider is None:
            return
    ...
    def _commit_slider_value(self, n):
        ...
        if self.init or self._syncing_from_artisan:
            return
        if self._slider_commit_epochs.pop(n, self._slider_command_epoch) \
                != self._slider_command_epoch:
            _log.warning('slider %s: stale command dropped (heat cut)', n)
            return
        if n in [0,1,2,3]:
