...
from PyQt6.QtGui import QColor, QCloseEvent, QDrag, QStandardItemModel
...
class AlarmSet(QObject):
    ...
    def validate(self) -> list[str]:
        ...
        for a in self._rows:
            ...
            # Artisan gates the IF ALARM anchor on a POSITIVE guard: with only
            # a "not fired" target the alarm is never even eligible, whatever
            # its offset or condition.
            if a.is_conditional and a.guard_aid is None:
                if a.negguard_aid is None:
                    warnings.append(
                    ...
                    warnings.append(
                        QApplication.translate(
                            'tilauscope_alarms',
                            'Alarm #{0}: never fires (a "not fired" target needs a starting event)')
                        .format(num))
...
def _hint(text: str) -> QLabel:
    """A wrapped explanatory line under a closed-off control."""
    lbl = QLabel('⚠  ' + text)
...
class _AlarmRow(QFrame):
    ...
    def _render_standard(self, a: Alarm) -> None:
        ...
        self._render_delay(a, 'time')
        self._render_condition(a)
        self._render_guard(a)
    ...
    def _render_conditional(self, a: Alarm) -> None:
        ...
        self._render_delay(a, 'ref')
        self._render_condition(a)
        self._render_action(a)

    def _render_delay(self, a: Alarm, cluster: str) -> None:
        # Offset 0 is not "fire at the anchor": the engine gates the time
        # criterion on offset > 0, so no delay is set at all. Render it as the
        # same "add" affordance as a missing condition rather than as +0:00.
        if a.offset > 0:
            self._lay.addWidget(self._chip(self._offset_text(a.offset), 'time', cluster))
        else:
            self._lay.addWidget(self._chip(
                '+ ' + QApplication.translate('tilauscope_alarms', 'delay'),
                'ghost', cluster, ghost=True))

    def _render_condition(self, a: Alarm) -> None:
        # Delay and condition are ORed by the engine, whichever comes first —
        # hence "or as soon as". With no delay the condition is the only
        # trigger and there is no alternative to announce.
        if a.source == -3:
            self._lay.addWidget(self._chip(
                '+ ' + QApplication.translate('tilauscope_alarms', 'condition'),
                'ghost', 'cond', ghost=True))
            return
        self._lay.addWidget(_kw(
            QApplication.translate('tilauscope_alarms', 'or as soon as') if a.offset > 0
            else QApplication.translate('tilauscope_alarms', 'as soon as')))
        chip = self._chip(self._cond_text(a), 'cond', 'cond')
        if a.is_conditional:
            chip.setToolTip(QApplication.translate(
                'tilauscope_alarms',
                'Measured against the reading taken when the guard fired, '
                'not an absolute temperature.'))
        self._lay.addWidget(chip)

    def _render_guard(self, a: Alarm) -> None:
        # A guard on an anchored alarm is an extra gate on top of the anchor.
        # Shown on every standard row so one loaded from a profile cannot act
        # without appearing in the sentence.
        if a.guard_aid is None and a.negguard_aid is None:
            self._lay.addWidget(self._chip(
                '+ ' + QApplication.translate('tilauscope_alarms', 'guard'),
                'ghost', 'ref', ghost=True))
            return
        self._lay.addWidget(_kw(QApplication.translate('tilauscope_alarms', 'only if')))
        self._lay.addWidget(self._chip(self._ref_text(a), 'ref', 'ref'))
    ...
    def _cond_text(self, a: Alarm) -> str:
        ...
        if a.is_conditional:
            # The engine subtracts the reading taken when the guard fired, so
            # the threshold is a change, not a temperature. Signed, and said.
            return QApplication.translate(
                'tilauscope_alarms', '{0} {1} {2} since then').format(
                    src, op, f'{a.value:+g}')
        return f'{src} {op} {a.value:g}'
...
class _ChipEditOverlay(QFrame):
    ...
    def _build_ref(self, a: Alarm) -> None:
        modes = [QApplication.translate('tilauscope_alarms', 'no guard'),
                 QApplication.translate('tilauscope_alarms', 'fired'),
        ...
        if a.guard_aid is not None:
            ...
            mode_idx = 0
        mode = self._combo(modes, mode_idx)
        # An IF ALARM alarm is started by its guard: Artisan only makes it
        # eligible on a POSITIVE guard, so the two other forms would produce a
        # rule that can never fire. Close them here instead of accepting one.
        if a.is_conditional:
            self._disable_mode(mode, 0)
            self._disable_mode(mode, 2)
        self._field(QApplication.translate('tilauscope_alarms', 'when target'), mode)
        if a.is_conditional:
            self._lay.addWidget(_hint(QApplication.translate(
                'tilauscope_alarms',
                '"Not fired" and "no guard" need a starting event — '
                'set one under timing.')))
        ...
        def _apply() -> None:
            ...
            if idx == 0 or not self._ref_aids:   # no guard
        ...
        # A conditional with nothing stored opens on the only legal form and
        # commits it: the target combo already shows a candidate, and with a
        # single one its index never changes, so no signal would ever fire and
        # the alarm would keep a guard the panel claims it has.
        if a.is_conditional and cur is None:
    ...
    @staticmethod
    def _disable_mode(combo: QComboBox, index: int) -> None:
        """Grey out one entry, keeping it visible so the operator sees the
        option exists and reads why it is closed here."""
        model = combo.model()
...
class TilauAlarmDlg(QDialog):
    ...
    def _build_audit_payload(self) -> tuple[str, str]:
        ...
        for i, a in enumerate(rows):
            ...
            if a.is_conditional:
                if a.guard_aid is not None:
                    ...
                    # No positive guard: the engine never makes this eligible.
                    # Spell that out or the audit reads it as a valid rule.
                    timing = (f'conditional with NO positive guard (never fires), '
                              f'only a "not fired" reference to alarm '
                              f'#{self.alarms.display_number(a.negguard_aid)}, at +{a.offset}s')
                else:
            ...
            if a.source != -3:
                ...
                src_label = self.adapter.source_label(a.source)
                if a.is_conditional:
                    # IF ALARM compares the CHANGE since the guard fired.
                    cond = (f' [when {src_label} has changed by {sym} {a.value:+g} '
                            f'relative to its reading when the guard fired]')
                else:
                    cond = f' [when {src_label} {sym} {a.value:g}]'
        ...
        system = (
            ...
            'never fires. A "not fired" reference is a restriction added on top of a '
            'guard, never a trigger on its own.\n'
            '- IF-alarm thresholds are RELATIVE, not absolute: the sensor reading at the '
            'instant the guard fired is subtracted first, so "BT > 10" on an IF-alarm '
            'means "BT has risen 10 deg above what it read when the guard fired". Judge '
            'these against a plausible CHANGE over the interval, never against a roast '
            'temperature; an absolute-looking threshold there (e.g. 190) is a mistake to '
            'flag, as such a rise never happens.\n'
    ...
    def _close_dialog(self) -> None:
        ...
        try:
            ...
            # The poll runs every 2s; save() writes fired_idx back to
            # qmc.alarmstate. Without a last read, an alarm that fired since
            # the previous tick is saved as not fired and re-arms mid-roast.
            self._poll_fired_state()
