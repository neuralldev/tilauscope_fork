...
class TilauRoutineCheck(QDialog):
    ...
    def _build_cycle_section(self) -> None:
        ...
        for i, val in enumerate(tick_values):
            ...
            # Half-width end cells so each tick sits at its own fraction of the
            # bar (0 / 25 / 50 / 75 / 100 %) instead of at a cell centre.
            if i == 0:
                ...
                stretch = 2
            ticks_row.addWidget(lbl, stretch=stretch)
...
class AlogScanner(QObject):
    ...
    (QDateTime, weight_grams: int) tuples, one per roast after last_clean.
    Every recorded profile counts as a turn of the drum, whether or not the
    roast reached DROP; the same roast saved twice counts once.
    ...
    def run(self) -> None:
        ...
        try:
            if self.directory is not None and self.directory.is_dir():
                ...
                # Keyed by roast identity, not by file: the same roast saved
                ...
                for meta in records.values():
                    ...
                    # roastUUID is stamped once per roast and survives a re-save.
                    # Without one, the epoch stands in: the index gives a profile
                    # that lacks it the file's own mtime, so two files can never
                    # be merged by accident.
                    key: object = meta.roast_uuid or ('epoch', meta.roastepoch)
                    kept = unique.get(key)
                    if kept is not None and kept[2] >= meta.mtime:
                        continue  # same roast, older file — keep the latest save
                    unique[key] = (dt, round(meta.weight_in_g), meta.mtime)
                dates_with_weight = [(dt, w) for dt, w, _ in unique.values()]
