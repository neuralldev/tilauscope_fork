...
class TilauTC4BLE(QObject):
    ...
    def __init__(self, address: str, poll_s: float = _POLL_S,
        ...
        self._burner_echo_at = 0.0                 # monotonic ts of that read
        ...
        self._link_gen = 0                         # RX generation: notify callbacks of a dead link
    ...
    async def _connect(self) -> None:
        ...
        try:
            ...
            gen = self._invalidate_rx()
            await self._client.start_notify(
                FF01_NOTIFY, lambda ch, data, gen=gen: self._on_notify(ch, data, gen)
            )
    ...
    async def _verified_burner_off(self) -> bool:
        ...
        The echo is dropped first and only a reading that came back AFTER the
        command counts. Kept as it was, a zero left over from the previous
        connection satisfied the check without the controller having answered
        once — and the init this runs after is exactly what can put the output
        back to a high default.

        The receive buffer goes with it: a frame cut in half by the drop would
        otherwise be glued to the first frame of the new link, and the parser
        reads the OLD fields out of that composite line while stamping it now.
        """
        self._cmd_burner = 0
        self._burner_echo = None
        self._burner_echo_at = 0.0
        self._buf.clear()                          # no bytes from the dead link
        for attempt in range(_VERIFY_TRIES):
            ...
            if self._burner_echo == 0 and self._burner_echo_at > sent_at:
                _logd.info("TilauTC4BLE: burner verified off (attempt %d)", attempt + 1)
                return True
        _logd.error("TilauTC4BLE: no fresh zero echo after %d attempts "
                    "(last echo=%s)", _VERIFY_TRIES, self._burner_echo)
        return False
    ...
    # ── notify handling ─────────────────────────────────────────────────────
    def _invalidate_rx(self) -> int:
        """Open a new RX generation: empty the buffer, orphan older callbacks."""
        self._link_gen += 1
        self._buf.clear()
        return self._link_gen

    def _on_notify(self, _ch, data: bytearray, gen: Optional[int] = None) -> None:
        # A callback still queued from a previous link carries an old generation:
        # its bytes belong to a connection whose state says nothing about this one.
        if gen is not None and gen != self._link_gen:
            _logd.debug("TilauTC4BLE: dropping notify from generation %s", gen)
            return
    ...
    def _parse(self, line: bytes) -> None:
        ...
        try:
            ...
            self._burner_echo_at = time.monotonic()
        except (ValueError, IndexError):
            self._burner_echo = None
    ...
    def _on_disconnect(self, client) -> None:
        # A callback from a client already replaced belongs to a link that is
        # gone: the reconnect has taken its own RX generation by now, and
        # voiding here would orphan it — the new link would be subscribed under
        # a generation nothing accepts, so a controller answering zero is never
        # heard and the verification closes a healthy transport mid-roast.
        if client is not None and client is not self._client:
            _logd.debug("TilauTC4BLE: ignoring disconnect from a superseded client")
            return
        self._invalidate_rx()                      # nothing from this link counts any more
        if self._closing:
            self._connected = False
