...
class BrewAdvisor:
    ...
    def advise(self, inp: BrewInput, method_id: str, dose_g: Optional[float] = None) -> BrewRecipe:
        ...
        if method_id == "ESPRESSO":
            ...
            if espresso.pi_type == PIType.NONE:
                # Nothing to hold on this machine, so nothing to name: a
                # pre-infusion step here asks for a gesture that does not exist.
                # Zeroed AFTER total_time_s is settled, deliberately — the shot
                # keeps its length and the pull absorbs the seconds. Cutting them
                # out instead would leave every shot reading slow against the
                # plan, and the dial-in would walk the grind coarser to chase it.
                espresso.pi_seconds = 0
                espresso.pi_wet_s = 0
                espresso.pi_dwell_s = 0
