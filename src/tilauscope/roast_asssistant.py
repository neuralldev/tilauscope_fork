...
def _extract_uuid_from_beans_field(beans_text: str) -> str|None:
    ...
    if not beans_text:
        return None
    # Tolerant on the separator: roasts saved while the key was translated
    # carry "uuid : : <value>", and they must resolve without a rewrite.
    m = re.search(r"uuid\s*:[\s:]*([a-f0-9\-]{32,36})", beans_text, re.IGNORECASE)
...
_BURNER_SLIDER_DEFAULT = 3   # Artisan's heater slider, and the Skywalker one


def _burner_slider_idx(aw) -> int:
    """Index du slider burner — même résolution que la page preheat
    (config du PID de préchauffe, repli 3 quand rien n'est configuré).

    L'absence de config et l'index 0 sont deux choses différentes : le `or 3`
    d'origine renvoyait 3 pour un burner configuré sur le slider 0, index que
    le PID accepte explicitement, et les commandes partaient vers le mauvais
    actionneur.
    """
    try:
        cfg = getattr(aw, "tilauPreheatingPid", None)
        cfg = getattr(cfg, "cfg", None)
        if cfg is None:
            return _BURNER_SLIDER_DEFAULT
        idx = int(cfg.heater_slider)
    except (AttributeError, TypeError, ValueError):
        return _BURNER_SLIDER_DEFAULT
    return idx if 0 <= idx <= 3 else _BURNER_SLIDER_DEFAULT
...
class _IdlePage(QWidget):
    ...
    def set_notice(self, text: str | None) -> None:
        """Say why a start was refused, in place of the invitation.

        None puts the invitation back — the operator has fixed what was
        missing, and a warning that outlives its cause reads as a new one.
        """
        self._notice = text
        self._paint()
    ...
    def _paint(self) -> None:
        if self._notice:
            self._lbl.setText(self._notice)
            colour = THEME['YELLOW']
        else:
            colour = THEME['SURFACE2']
        ...
        self._lbl.setStyleSheet(
            f"color: {colour}; font-size: 16px; {_FONT} border: none;")
...
class RoastAssistantPanel(QWidget):
    ...
    def _build_ui(self) -> None:
        ...
        # plan confidence and machine write capability.
        ...
        # Connected here rather than with the setup bar itself: the slot
        # mirrors the selection in the header AND takes down a refused-start
        # notice, which the idle page has to exist to carry.
    ...
    def _start_assistant(self) -> None:
        ...
        if bean is None or agtron is None:
            ...
            # In Guided the recording itself starts the assistant, and the
            # button unchecked just above is hidden at that level: without a
            # word on the idle page the operator pressed START, the roast
            # recorded, and nothing said the assistant had nothing to follow.
            if bean is None and agtron is None:
                _notice = QApplication.translate("tilauscope_roast_assistant",
                    "Pick your green bean and roasting target above — the assistant "
                    "needs both before it can guide the roast.")
            elif bean is None:
                _notice = QApplication.translate("tilauscope_roast_assistant",
                    "Pick your green bean above — the assistant needs to know the "
                    "coffee before it can guide the roast.")
            else:
                _notice = QApplication.translate("tilauscope_roast_assistant",
    ...
    def connect_bridge(self, bridge: "RoastDataBridge") -> None:
        ...
        bridge.sample_ready.connect(self._on_sample_ready)
    ...
    @pyqtSlot()
    def _on_sample_ready(self) -> None:
        """Fin d'échantillon : BT, ET et RoR sont posés, la page peut calculer.

        Le rafraîchissement vivait dans _on_bt, donc une passe en avance sur
        l'ET et le RoR, et muette quand la BT ne bougeait pas.
        """
        if not self.is_active or self._current_phase == self._PHASE_IDLE:
            return
        self._refresh_current_page()
...
# ── Visibilité du panel, cacher le panneau par le bouton ────────────────────────────────────────────────────
    def hide_from_button(self):
        """✕ of the floating shell — sends the assistant back to its dock.

        There is no closed state to reach from here. Guided keeps the session
        and only moves the panel, and Expert has no panel to close: taking the
        level up to Expert takes the assistant down with it, so this button
        cannot be clicked at that level.
        """
        self.anchor_requested.emit()

    def stop_session(self) -> None:
        """End the guided session from outside — the level went to Expert,
        which has no assistant, so there is nothing left to advance."""
        if self.is_active:
            self._stop_assistant()
