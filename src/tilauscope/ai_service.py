...
class TilauAIService(QObject):
    ...
    def __init__(self, ai_config: TilauAIConfig, parent: QObject | None = None) -> None:
        ...
        # Active slots: task_type → (generation, QThread, _AIWorker, _CancelToken)
        # The generation tells two runs of the same task type apart: a cancelled
        # run frees its key at once, so a finished/error signal already queued
        # when it was cancelled would otherwise be taken for its successor's.
        self._active: dict[str, tuple[int, QThread, _AIWorker, _CancelToken]] = {}
    ...
    def submit(
        ...
        # Wire signals. Every callback carries the generation it belongs to and
        # is dropped when that run is no longer the one registered.
        thread.started.connect(worker.run)
        worker.token.connect(
            lambda chunk, tt=task_type, g=gen: self._on_worker_token(tt, g, chunk)
        )
        worker.finished.connect(
            lambda result, tt=task_type, g=gen: self._on_worker_finished(tt, g, result)
        )
    ...
    def cancel(self, task_type: str) -> None:
        ...
        if entry is None:
            return
        _gen, thread, _worker, cancel_token = entry
    ...
    def _is_current(self, task_type: str, gen: int) -> bool:
        """Whether `gen` is still the run registered under `task_type`.

        False for a callback from a cancelled run: acting on it would stop the
        task that took its place and publish a result nobody is waiting for.
        """
        entry = self._active.get(task_type)
        if entry is None:
            return False
        return entry[0] == gen

    def _on_worker_token(self, task_type: str, gen: int, chunk: str) -> None:
        if not self._is_current(task_type, gen):
            return
        self.token_received.emit(task_type, chunk)

    def _on_worker_finished(self, task_type: str, gen: int, result: Any) -> None:
        if not self._is_current(task_type, gen):
            _log.info("TilauAIService: late result for cancelled %s dropped", task_type)
            return
        self._cleanup(task_type)
        self.task_finished.emit(task_type, result)
        _log.info("TilauAIService: task %s finished", task_type)

    def _on_worker_error(self, task_type: str, gen: int, msg: str) -> None:
        if not self._is_current(task_type, gen):
            _log.info("TilauAIService: late error for cancelled %s dropped", task_type)
            return
        self._cleanup(task_type)
    ...
    def _cleanup(self, task_type: str) -> None:
        entry = self._active.pop(task_type, None)
        if entry:
            _gen, thread, _worker, _token = entry
            thread.quit()
