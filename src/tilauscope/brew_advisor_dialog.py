...
def _ai_number(data: dict, key: str, lo: float, hi: float) -> float:
    """One bounded number out of an AI payload, or 0.0.

    The JSON is only checked for syntax on the way in, so a field can hold
    anything at all. This runs on the task_finished slot: an exception here
    does not fall back to the base recipe, it leaves a Qt virtual and takes
    the application down with it.
    """
    try:
        return _clamp(float(data.get(key, 0) or 0), lo, hi)
    except (TypeError, ValueError):
        _log.info("Brew: AI returned an unusable %s (%r), ignored", key, data.get(key))
        return 0.0
...
class BrewAdvisorService(QObject):
    ...
    def add_correction(self, corr: BrewCorrection) -> None:
        ...
        if cur is None:
            # No dial-in yet does not mean the recipe was the raw
            # recommendation: the learned cross-bean offset may already be
            # folded in, and THAT is the cup the operator tasted. Composing on
            # the recommendation instead would undo the offset on the first
            # correction — and _store_dialin writes the result as an absolute
            # grind, so the mistake would reach the library.
            mult, _n = self.setup_offset_for(self._method)
            if mult is not None:
                cur = BrewCorrection(grind_mult=mult)
        if cur is None:
            composed = corr
        else:
            ...
            if m:
                ...
                if old_w > 0 and rec.steps:
                    ...
                    for st in rec.steps:
                        ...
                        # The step SENTENCE is rendered from params, not from the
                        # target: leaving it behind prints "Pour to 240 g" beside
                        # a 225 g pill, in the same row and on the label.
                        if "g" in st.params:
                            st.params["g"] = round(float(st.params["g"]) * k, 1)
                    # Same reason for the bloom chip, which is read off the recipe
                    # rather than off its step.
                    if rec.bloom_g > 0:
                        rec.bloom_g = round(rec.bloom_g * k, 1)
    ...
    def recompute(self) -> None:
        ...
        # Presence, not content: a cup tasted BALANCED carries no correction and
        # is still a dial-in. Reading it as "never dialled" would hand this bean
        # to the cross-bean offset and move a setting the operator validated.
        if corr is not None:
    ...
    def apply_ai(self, data: dict) -> bool:
        ...
        td = _ai_number(data, "temp_delta_c", -2.0, 2.0)
        gp = _ai_number(data, "grind_pct", -10.0, 10.0)
        rd = _ai_number(data, "ratio_delta", -1.0, 1.0)
        corr = BrewCorrection(
...
class BrewAdvisorDlg(QDialog):
    def __init__(self, inp: BrewInput, title: str = "", aw=None, beancave=None, bean=None):
        ...
        self._ai_busy = False
        # Fingerprint of the recipe an in-flight AI request was built on; the
        # answer is refused if it no longer matches. None = nothing in flight.
        self._ai_req_key: Optional[tuple] = None
        # live-brew state
        ...
        # Closing releases the scale, the timer and the AI connections exactly
        # once, whichever gesture closes the window.
        self._cleaned = False
    ...
    def _run_ai(self) -> None:
        ...
        if svc.submit(_BREW_TASK, _work):
            # Only the AI button is disabled while this runs: method, dose,
            # machine and style stay live. Remember what was actually sent.
            self._ai_req_key = self._recipe_key(rec)
            self._ai_busy = True
            if self.btn_ai:
                self.btn_ai.setEnabled(False)
    ...
    @staticmethod
    def _recipe_key(rec) -> Optional[tuple]:
        """What makes an AI refinement valid for a recipe and not another one.

        Everything the request describes and the operator can still change
        while it is in flight: the method, the puck or the bed, and the numbers
        the model was asked to nudge.
        """
        if rec is None:
            return None
        return (rec.method_id, round(float(rec.dose_g), 1), int(rec.grind_um),
                round(float(rec.temp_c), 1), rec.ratio_str, rec.roast_key)
    ...
    def _on_ai_finished(self, task_type: str, result: object) -> None:
        ...
        if self._brew_running:
            ...
            return
        if self._recipe_key(self.service.last) != self._ai_req_key:
            # The operator moved on while it was in flight. A refinement reasoned
            # on a V60 has no business adjusting the espresso now on screen.
            _log.info("Brew: AI refinement discarded, the recipe changed since the request")
            return
    ...
    def _load_dialins(self) -> None:
        ...
        for d in (getattr(self._bean, "dial_ins", None) or []):
            try:
                ...
                # Stored even when it is neutral: the entry says this bean was
                # dialled on this method, which is what vetoes the setup offset.
    ...
    def _store_dialin(self, rec, dg, tastes: list[str],
        ...
        try:
            ...
            # Register it in the session exactly as reopening would rebuild it
            # from the stored absolutes. Matters for a cup tasted BALANCED: no
            # correction was composed, so without this the bean stays "never
            # dialled" until the dialog is reopened — and the cross-bean offset
            # keeps owning it.
            self.service.set_dialin(entry.method_id, BrewCorrection(
                grind_mult=(entry.grind_um / base.grind_um) if (base and base.grind_um) else 1.0,
                ratio_delta=(entry.ratio - entry.base_ratio) if (entry.ratio and entry.base_ratio) else 0.0,
                temp_delta=(entry.temp_c - entry.base_temp_c) if (entry.temp_c and entry.base_temp_c) else 0.0))
    ...
    def _persist_bean(self) -> bool:
        ...
        try:
            ...
            return False
        # BeanCave reports its own write failures to the operator and returns
        # normally, so "no exception" is not "saved". An older host that returns
        # nothing keeps the previous, optimistic reading.
        if written is False:
            _log.error("Brew: BeanCave refused the save; dial-in kept in memory only")
            return False
        return True
    ...
    def _cleanup(self) -> None:
        """Release everything this dialog took, once.

        Reached from both closeEvent and done(): the cross, the Close button
        and Escape all go through accept/reject, which never raises a close
        event. Idempotent because QDialog.closeEvent itself falls back to
        reject(), so both paths fire on a window-manager close.
        """
        if self._cleaned:
            return
        self._cleaned = True
        try:
            ...
            # Raised from a Qt virtual, this would take the application down.
            _log.error("Brew: cleanup on close failed: %s", exc)
    ...
    def done(self, r: int) -> None:
        # accept(), reject() and Escape all land here and nowhere near closeEvent.
        self._cleanup()
        super().done(r)
