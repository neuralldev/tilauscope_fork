...
def content() -> str:
    ...
    tbl_Commands.add_row(['&#160;','zoom(<x>[,<y>])',QApplication.translate('HelpDlg','zooms the canvas by factor <x> horizontally and the optional <y> vertically. <y> defaults to <x>.')])
    tbl_Commands.add_row(['&#160;','pan(<x>[,<y>])',QApplication.translate('HelpDlg','pans the canvas by factor <x> horizontally and the optional <y> vertically. A factor 0 does not apply any panning, a factor 1 moves by one full screen to the left/bottom, a factor -1 by one full screen to the right/top. <y> defaults to <x>.')])
    tbl_Commands.add_row(['&#160;','center(<x>,<y>[,<clamp>])',QApplication.translate('HelpDlg','centers canvas and then applies panning according to the given x/y parameters in horizontal and vertical direction. 0: no panning, 1: pan canvas one full screen to the left/bottom, -1: pan canvas one full screen to the right/top. If <clamp> is 0 the canvas is centered w.r.t. both axis limits. If not recording, and <clamp> i -1 the canvas is centered only horizontally wr.t. its x-axis limits and with clamp in {1,..,4} w.r.t. to both axis limits. While recording if <clamp> is -1 the canvas is centered horizontally to the current time, if 1 also centered to current BT, 2 current ET, 3 current BT RoR, 4 current ET RoR. <clamp> defaults to -1 if xy-cursor widget is clamped to any temp or RoR curve (z-key), 0 if zoom follow is OFF and xy-cursor not clamped, 1-4 if zoom follow is ON and xy-cursor not clamped, centering on the value zoom followed.')])
    tbl_Commands.add_row(['&#160;','clamp(<n>)',QApplication.translate('HelpDlg','set xy cursor clamp mode (shortcut key z) to 0: off, 1: BT@x, 2: ET@x, 3: BTB@x, 4: ETB@x')])
    tbl_Commands.add_row(['&#160;','followMode(<bool>)',QApplication.translate('HelpDlg','enables/disables follow mode')])
    tbl_Commands.add_row(['&#160;','followModePanning(<x>[,<y>])',QApplication.translate('HelpDlg','sets additional panning in horizontal and vertical direction, applied in follow mode after the centering. 0: no panning, 1: pan canvas one full screen to the left/bottom, -1: pan canvas one full screen to the right/top. <y> defaults to <x>.')])
    tbl_Commands.add_row(['&#160;','home',QApplication.translate('HelpDlg','toolbar home action')])
    tbl_Commands.add_row(['&#160;','back',QApplication.translate('HelpDlg','toolbar back action')])
    tbl_Commands.add_row(['&#160;','forward',QApplication.translate('HelpDlg','toolbar forward action')])
