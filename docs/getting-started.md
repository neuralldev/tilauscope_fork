...
assistant steps aside entirely.
...
Leaving Guided mid-roast ends the guidance for that roast: the plan is not held in reserve, and
the roast finishes on your own reading of the machine.

The assistant belongs to Guided. There, it can be **docked** in place of the control panel or
**detached** into its own floating window, with the ⤢ button in the panel's own header, and
closing the detached window sends it back to the dock with its roast in progress untouched.
Expert has no assistant: taking the level up to Expert takes the panel down, docked or
detached, and gives you the control panel back.
